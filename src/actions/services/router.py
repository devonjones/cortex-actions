...
class Router:
    ...
    def _ensure_schema(self) -> None:
        ...
        # this router does in production is drain a 83k-row backfill into
        # `school`; without this index each of those enqueues scans every
        # pending row already there, which is the quadratic shape that looks
    ...
    def _route(self, conn: Any, job: dict[str, Any]) -> str:
        ...
        if not isinstance(payload, dict):
            ...
            # being retried, and reaching `.get` on it would raise into the
            # handler -- which is how a row nobody can fix reaches dead_letter.
            payload = {}
        label = payload.get("label")
        event_type = payload.get("event_type", "added")
        gmail_id = payload.get("gmail_id")

        # isinstance, not truthiness. A non-str gmail_id passes `not gmail_id`
        # and then makes enqueue's dedup validation raise QueueError, which the
        # handler turns into fail_or_retry and eventually dead_letter -- so two
        # equally unfixable rows took opposite paths, one dropped cleanly and
        # one buried among the real failures.
        if not isinstance(label, str) or not isinstance(gmail_id, str):
            # Malformed rows cannot become well-formed by being retried.
            logger.error(
                "dropping malformed action event", job_id=job.get("id"), payload=payload
            )
        ...
        for queue_name in queues:
            ...
            # as ROUTED would have made the backfill look like it enqueued 83k
            # jobs when it enqueued far fewer, which is the number we intend to
            # read before switching the producer on.
            job_id = enqueue(
                conn,
                queue_name,
    ...
    def _complete(self, conn: Any, job: dict[str, Any], outcome: str) -> str:
        """Settle one claimed job, and count it. THE ONLY PLACE EITHER HAPPENS.

        Three paths settle a job cleanly -- routed, unmatched, dropped -- and
        each used to carry its own copy of complete/commit/rollback and its own
        decision about whether to touch QUEUE_PROCESSED. Only one of the three
        did, so the queue's own throughput metric counted a third of the
        traffic and the two paths that handle the majority of events were
        invisible.
        """
        if not complete(conn, job["id"], self._worker, commit=False):
            ...
            QUEUE_PROCESSED.labels(queue=SOURCE_QUEUE, status="lost").inc()
            logger.warning(
        ...
        conn.commit()
        QUEUE_PROCESSED.labels(queue=SOURCE_QUEUE, status="success").inc()
        return outcome
    ...
    def process_job(self, job: dict[str, Any]) -> str:
        ...
        try:
            ...
            #
            # CheckViolation is in here for a specific reason: enqueue with
            # commit=False deliberately gives up partition self-heal, because
            # creating a partition needs a commit that would commit the caller's
            # pending work. A router up across a day boundary with the
            # maintenance pass broken hits it on every enqueue -- an
            # infrastructural fault, with perfectly healthy events.
    ...
    def _settle(
        ...
        """
        job_id = job.get("id")
        ERRORS.labels(service=SERVICE, error_type=kind).inc()
        try:
            conn.rollback()
            if kind == "released":
                logger.warning(
                ...
                release(conn, job["id"], self.poll_interval, self._worker)
            else:
                logger.error("routing failed", job_id=job_id, error=str(exc)[:300])
                fail_or_retry(conn, job["id"], str(exc), self._worker)
        except Exception as settle_exc:  # noqa: BLE001 - see the docstring
            # Deliberately NOT counted in QUEUE_PROCESSED: nothing was
            # processed. The row is still `processing` and comes back when its
        ...
        QUEUE_PROCESSED.labels(queue=SOURCE_QUEUE, status=kind).inc()
        return kind
    ...
    def run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            # switched on and the 83k backfill starts moving.
            logger.info(
                "batch",
                claimed=len(jobs),
